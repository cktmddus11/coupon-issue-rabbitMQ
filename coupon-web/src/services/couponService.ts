...
export const couponService = {
  ...
  async issueCoupon(request: CouponRequest): Promise<CouponResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || '쿠폰 발급 처리 중 오류가 발생했습니다.');
      }
      ...
      return {
        ...
        success: false,
        message: error instanceof Error ? error.message : '쿠폰 발급 요청 중 오류가 발생했습니다.'
      };
    }
  },
  ...
  async getUserCoupons(userId: string): Promise<Coupon[]> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_PATH}/coupons/user/${userId}`);
  ...
  async getValidUserCoupons(userId: string): Promise<Coupon[]> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_PATH}/coupons/user/${userId}/valid`);
